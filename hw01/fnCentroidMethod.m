function [ v_nearest_class ] = fnCentroidMethod( mat_test, mat_train, v_class )
...
%     mat_point = [6,6];
    ...
    if(size(mat_test,2) ~= size(mat_train,2))
        error(['Dimensions in test data (',size(mat_test,2),') is not the same as training data (',size(mat_training,2),')']);
    end
